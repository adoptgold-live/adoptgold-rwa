...
<head>
  ...
  <link rel="stylesheet" href="/rwa/assets/css/rwa-design-system.css?v=20260316-final-profile-v4.1">
  <style>
    :root{
...
(() => {
  ...
  const I18N = {
    en: {
      ...
      loading: 'Loading...',
      tonPreparing: 'Preparing TON bind...',
      tonWaitingWallet: 'Waiting approval of wallet...',
      tonResetOk: 'TON session reset.',
      tonResetFailed: 'TON reset failed.',
      tonInvalidAddress: 'Invalid TON address',
      tonPostingProof: 'Posting proof',
      tonProofVerified: 'Proof verified',
      tonBindOk: 'TON wallet bound.',
      tonBindFailed: 'TON bind failed.',
      saveFailed: 'Profile submit failed.',
      tonConnectedResetFirst: 'Wallet already connected. Reset TON first, then bind again.',
      tonPreparingFresh: 'Preparing fresh TON bind session...',
      tonRebindGuide: 'Open mobile wallet, switch to the target TON address, then approve.',
      tonResetClientOk: 'TON client session cleared.',
      tonWaitingProof: 'Waiting wallet proof...',
      tonSubmittingProof: 'Submitting proof...',
      tonSavedOk: 'TON wallet saved successfully.',
      tonOpenWallet: 'Open wallet and approve connection.'
    },
    zh: {
      pageTitle: '资料',
      ...
      selectArea: '选择地区',
      loading: '加载中...',
      tonPreparing: '正在准备 TON 绑定...',
      tonWaitingWallet: '等待钱包批准...',
      tonResetOk: 'TON 会话已重置。',
      tonResetFailed: 'TON 重置失败。',
      tonInvalidAddress: '无效 TON 地址',
      tonPostingProof: '正在提交证明',
      tonProofVerified: '证明已验证',
      tonBindOk: 'TON 钱包已绑定。',
      tonBindFailed: 'TON 绑定失败。',
      saveFailed: '资料提交失败。',
      tonConnectedResetFirst: '钱包仍然已连接，请先重置 TON，再重新绑定。',
      tonPreparingFresh: '正在准备全新的 TON 绑定会话...',
      tonRebindGuide: '请打开手机钱包，先切换到目标 TON 地址，再批准连接。',
      tonResetClientOk: 'TON 客户端会话已清除。',
      tonWaitingProof: '等待钱包证明...',
      tonSubmittingProof: '正在提交证明...',
      tonSavedOk: 'TON 钱包已成功保存。',
      tonOpenWallet: '请打开钱包并批准连接。'
    }
  };
  ...
  function placeholderSvg(letter) {
    const l = String(letter || 'U').trim().charAt(0).toUpperCase() || 'U';
    return svgDataUrl(`
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 160 160">
        ...
        <rect width="160" height="160" rx="34" fill="#0d0915"/>
        <rect x="8" y="8" width="144" height="144" rx="28" fill="url(#g)" opacity=".16"/>
        <text x="50%" y="54%" text-anchor="middle" font-size="68" font-family="ui-monospace, Menlo, Consolas, monospace" fill="#f7efff">${l}</text>
      </svg>
    `);
  }

  function tonIdenticonSvg(seed) {
    const s = String(seed || '').trim();
    if (!s) return placeholderSvg('U');
    ...
    const hue = h % 360;
    const c1 = `hsl(${hue} 82% 62%)`;
    const c2 = `hsl(${(hue + 58) % 360} 78% 56%)`;
    const c3 = `hsl(${(hue + 130) % 360} 72% 50%)`;
    ...
    for (let row = 0; row < 5; row++) {
      for (let col = 0; col < 3; col++) {
        const b = bytes[row * 5 + col];
        if (b % 2 === 0) continue;
        const fill = [c1, c2, c3][b % 3];
        const x1 = 18 + col * (size + gap);
        const y1 = 18 + row * (size + gap);
        const mx = 18 + (4 - col) * (size + gap);
        cells.push(`<rect x="${x1}" y="${y1}" width="${size}" height="${size}" rx="7" fill="${fill}" />`);
        if (col !== 2) cells.push(`<rect x="${mx}" y="${y1}" width="${size}" height="${size}" rx="7" fill="${fill}" />`);
      }
    }
    ...
    return svgDataUrl(`
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 160 160">
        ...
        <rect x="8" y="8" width="144" height="144" rx="28" fill="none" stroke="rgba(255,255,255,.08)"/>
        ${cells.join('')}
      </svg>
    `);
  }

  function renderAvatar(walletAddress, nickname) {
    const wallet = String(walletAddress || '').trim();
    const nick = String(nickname || '').trim();
    el.profileAvatar.src = wallet ? tonIdenticonSvg(wallet) : placeholderSvg(nick || 'U');
  }
  ...
  function applyProfile(user) {
    ...
    renderAvatar(user.wallet_address || '', user.nickname || '');
  ...
  async function initProfileSequence() {
    ...
    if (!json.ok || !json.user) throw new Error(json.message || t('saveFailed'));

    await loadCountries();
    await loadPrefixes();

    applyProfile(json.user);
  ...
  async function submitProfile(ev) {
    ...
    try {
      ...
      setProfileStatus(e.message || t('saveFailed'), 'bad');
      toggleSubmitAvailability();
    } finally {
  ...
  function setLang(lang) {
    ...
    if (!state.profile) {
      ...
      el.proofStateText.textContent = t('tonIdle');
      el.avatarSub.textContent = t('avatarPlaceholder');
    } else {
      renderAvatarMeta(!!state.profile.wallet_address);
  ...
  function installTonProofClient() {
    ...
    state.tonProofClient = window.RwaTonProof.create({
      ...
      async onProofReady({ address, proof }) {
        ...
        const finalAddress = String(json.wallet_address || address);
        ...
        setTonBadge(true);
        setTonValidation(t('tonValidationOk'), 'ok');
        setTonStatus(json.message || t('tonSavedOk'), 'ok');
        el.proofStateText.textContent = t('tonProofVerified');

        renderAvatar(finalAddress, el.nickname.value || state.profile?.nickname || '');
  ...
  function installEvents() {
    ...
    el.nickname.addEventListener('input', () => {
      renderAvatar(state.profile?.wallet_address || '', el.nickname.value);
      saveDraft();
      clearTimeout(state.nicknameTimer);
      state.nicknameTimer = setTimeout(() => checkNicknameAvailable(el.nickname.value, true), 400);
    });
    ...
    el.resetTonBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        setTonValidation(state.profile?.wallet_address ? t('tonValidationOk') : t('tonValidationMissing'),
          state.profile?.wallet_address ? 'ok' : 'warn');
        setTonStatus(t('tonResetClientOk'), 'ok');
      } catch (e) {
