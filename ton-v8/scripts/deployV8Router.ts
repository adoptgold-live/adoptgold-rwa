...
import { V8MintRouter, type V8MintRouterConfig } from '../wrappers/V8MintRouter.ts';
...
export async function run(provider: NetworkProvider) {
  ...
  const routerCode = loadCompiledCodeCell('./build/v8_mint_router.compiled.json');
  ...
  const router = V8MintRouter.createFromConfig(config, routerCode);

  console.log('========================================');
  console.log('V8 ROUTER DEPLOY');
  console.log('========================================');
  console.log('Address:', router.address.toString());
