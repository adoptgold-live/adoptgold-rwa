...
import { V8NftCollection, type V8NftCollectionConfig } from '../wrappers/V8NftCollection.ts';
...
function loadCompiledCodeCell(path: string): Cell {
  ...
  const hex =
    ...
    parsed?.result?.hex ??
    '';
  if (!hex) throw new Error(`Invalid compiled artifact: ${path}`);
...
export async function run(provider: NetworkProvider) {
  ...
  const collectionCode = loadCompiledCodeCell('./build/v8_nft_collection.compiled.json');
  const itemCode = loadCompiledCodeCell('./build/v8_nft_item.compiled.json');
  ...
  const collection = V8NftCollection.createFromConfig(config, collectionCode);

  console.log('========================================');
  console.log('V8 COLLECTION DEPLOY');
  console.log('========================================');
