<?php
// /var/www/html/public/rwa/auth/email/verify-email.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $endpoint = 'https://' . ($_SERVER['HTTP_HOST'] ?? 'adoptgold.app') . '/rwa/auth/email/send-verify.php';
    ...
    if ($curlErr) {
        $msg = 'Unable to send verification email: ' . $curlErr;
        $msgType = 'err';
    } else {
        ...
        if (is_array($json) && !empty($json['ok'])) {
            ...
            $msg = is_array($json) ? (string)($json['error'] ?? 'Unable to send verification email.') : 'Unexpected server response.';
            $msgType = 'err';
        }
    }
}
...
:root{
  ...
  --gold:#ffd86b;--green:#4cffb2;--red:#ff637e;--shadow:0 0 28px rgba(182,108,255,.12);
}
    ...
    <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;">
      <div>
        ...
        <div class="sub" style="margin-top:8px;">
          Your standalone RWA profile email is used for notices and recovery. Open the verification link from your inbox to complete verification.
        </div>
      </div>
    ...
    <div class="actions">
      <?php if (!$isVerified): ?>
        <form method="post" style="margin:0;">
          <button type="submit" class="btn gold">SEND VERIFY EMAIL</button>
        </form>
      <?php endif; ?>
