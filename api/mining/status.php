...
/**
 * /var/www/html/public/rwa/api/mining/status.php
 * Mining runtime hydrator
 */
...
function mining_status_json(array $a, int $code = 200): never
{
...
function mining_status_fail(string $error, string $message, int $code = 400, array $extra = []): never
{
    mining_status_json(array_merge([
        'ok' => false,
        'error' => $error,
        'message' => $message,
    ], $extra), $code);
...
function mining_status_wallet(array $user): string
{
    return trim((string)($user['wallet_address'] ?? $user['wallet'] ?? ''));
}

function mining_status_gate(PDO $pdo, int $userId, string $wallet): array
{
...
function mining_status_fallback_tier(PDO $pdo, int $userId): array
{
    $isVerified = 0;
    try {
...
function mining_status_resolve(PDO $pdo, int $userId, string $wallet): array
{
    ...
    return mining_status_fallback_tier($pdo, $userId);
}

function mining_status_profile_totals(PDO $pdo, int $userId, string $wallet): array
{
    try {
        $st = $pdo->prepare("
            SELECT
...
function mining_status_booster(array $tier): array
{
...
function mining_status_payload(PDO $pdo, array $user): array
{
    $userId = (int)($user['id'] ?? 0);
    $wallet = mining_status_wallet($user);

    $gate = mining_status_gate($pdo, $userId, $wallet);
    $tier = mining_status_resolve($pdo, $userId, $wallet);
    $totals = mining_status_profile_totals($pdo, $userId, $wallet);

    $runtime = $_SESSION['rwa_mining'] ?? [];
    $isMining = (int)($runtime['is_mining'] ?? 0);
    $batteryPct = (float)($runtime['battery_pct'] ?? 0);
    ...
    return [
        ...
        'booster' => mining_status_booster($tier),
    ];
}
...
if (!$pdo instanceof PDO) {
    mining_status_fail('NO_DB', 'DB connection unavailable', 500);
}

$user = session_user();
if (!is_array($user) || empty($user)) {
    mining_status_fail('NO_SESSION', 'Login required', 401);
}
...
mining_status_json(mining_status_payload($pdo, $user));
