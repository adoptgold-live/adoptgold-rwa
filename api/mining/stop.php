...
/**
 * /var/www/html/public/rwa/api/mining/stop.php
 * Canonical runtime stop endpoint.
 *
 * FIXES:
...
function mining_stop_json(array $a, int $code = 200): never
{
...
function mining_stop_fail(string $error, string $message, int $code = 400): never
{
    mining_stop_json([
        'ok' => false,
        'error' => $error,
...
function mining_stop_wallet(array $user): string
{
    return trim((string)($user['wallet_address'] ?? $user['wallet'] ?? ''));
}

function mining_stop_gate(PDO $pdo, int $userId, string $wallet): array
{
...
function mining_stop_fallback_tier(PDO $pdo, int $userId): array
{
    $isVerified = 0;
    try {
    ...
    if ($isVerified === 1) {
        return ['tier' => 'verified', 'tier_label' => 'Verified Miner', 'tier_status' => 'active', 'multiplier' => 2.0, 'daily_cap_wems' => 300.0, 'binding_cap' => 10, 'ema_staked_active' => 0.0, 'ema_stake_expires_at' => null, 'node_reward_pct' => 0.0];
    }

    return ['tier' => 'free', 'tier_label' => 'Free Miner', 'tier_status' => 'active', 'multiplier' => 1.0, 'daily_cap_wems' => 100.0, 'binding_cap' => 0, 'ema_staked_active' => 0.0, 'ema_stake_expires_at' => null, 'node_reward_pct' => 0.0];
}

function mining_stop_resolve_tier(PDO $pdo, int $userId, string $wallet, array $user): array
{
    ...
    return mining_stop_fallback_tier($pdo, $userId);
}

function mining_stop_totals(PDO $pdo, int $userId, string $wallet): array
{
    try {
        $st = $pdo->prepare("
            SELECT
...
function mining_stop_payload(PDO $pdo, array $user): array
{
    $userId = (int)($user['id'] ?? 0);
    $wallet = mining_stop_wallet($user);
    $gate = mining_stop_gate($pdo, $userId, $wallet);
    $tier = mining_stop_resolve_tier($pdo, $userId, $wallet, $user);
    $totals = mining_stop_totals($pdo, $userId, $wallet);
    ...
    return [
        ...
        'rate_wems_per_tick' => round(0.33 * (float)$tier['multiplier'], 8),
...
if (!is_array($user) || empty($user)) {
    mining_stop_fail('NO_SESSION', 'Login required', 401);
}
...
if (!$pdo instanceof PDO) {
    mining_stop_fail('NO_DB', 'DB connection unavailable', 500);
}

mining_stop_json(mining_stop_payload($pdo, $user));
