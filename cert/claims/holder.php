...
<title>Holder Claims · POAdo RWA</title>
<style>
:root{
    --bg:#050505;
    --panel:#0f0f0f;
...
<div class="wrap">
    ...
    <div class="topbar">
        <div>
            <div class="title">Holder Claims</div>
    ...
    <div class="panel">
        ...
        <div class="note">
            This page reads from the canonical holder claim ledger. Claim settlement records a treasury/admin payout transaction hash and updates the ledger status.
        </div>
    </div>

    <div class="panel">
        <div class="panel-title">Claim Rows</div>
    ...
    <div class="panel">
        <div class="panel-title">Claim Settlement</div>
        <div class="toolbar">
        ...
        <div class="note">
            This action does not broadcast a blockchain transaction. It records settlement in the ledger after the treasury/admin payout already exists.
        </div>
    </div>
...
(function () {
    ...
    const CFG = window.POADO_HOLDER_CLAIMS || {};
    const API = CFG.api || {};
    const CSRF = CFG.csrf || {};
    ...
    function log(msg, level) {
        ...
        row.textContent = `[${ts}] ${level ? '[' + level + '] ' : ''}${msg}`;
    ...
    function renderRows(items) {
        if (!items || !items.length) {
            el.claimRows.innerHTML = '<tr><td colspan="10">No claim rows found.</td></tr>';
            return;
        }
    ...
    async function loadClaims() {
        ...
        log('Loading holder claims...', 'INFO');
    ...
    async function claimSelected() {
        ...
        if (!claimUids.length) {
            log('No claim rows selected.', 'WARN');
            return;
        }
        if (!claimedTxHash) {
        ...
        log(`Submitting ${claimUids.length} holder claim(s)...`, 'INFO');

        const payload = {
            claim_uids: claimUids,
        ...
        if (CSRF.claimHolder) {
            fd.append('csrf_token', CSRF.claimHolder);
        }
        ...
        log(`Claim settlement processed. Updated=${data.updated_count || 0}`, 'OK');
        await loadClaims();
    }
