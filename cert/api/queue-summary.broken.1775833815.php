...
function qs_build_mint_meta(array $certMeta): array
{
    ...
    // Must be in active mint step
    if (!in_array($flowState, ['minting'], true)) {
        return false;
    }

    if (!in_array($step, ['wallet_sign', 'submitted'], true)) {
        return false;
    }

    // TIME CHECK (CRITICAL)
    $events = $handoff['events']['dispatch'] ?? [];
    $lastAt = $handoff['handoff']['at'] ?? '';

    if ($lastAt) {
        $ts = strtotime($lastAt);
        if ($ts && (time() - $ts > 300)) { // 5 minutes
