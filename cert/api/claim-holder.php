 ...
 * File: /var/www/html/public/rwa/cert/api/claim-holder.php
 *
 * Purpose:
 * - Claim holder TON rewards from holder claim ledger
 * - Reads/writes:
 *     wems_db.poado_rwa_holder_claims
 *
 * Current design:
 * - Owner-only
 * - Claim one or multiple holder claim rows
 ...
 *
 * Assumed holder claim ledger table:
 *   poado_rwa_holder_claims
 *
 * Fields used:
 *   id
 *   claim_uid
...
if (!function_exists('poado_claim_holder_exit')) {
    function poado_claim_holder_exit(array $payload, int $status = 200): void
    {
...
if (!function_exists('poado_claim_holder_norm_decimal')) {
    function poado_claim_holder_norm_decimal($value, int $scale = 9): string
    {
        if ($value === null || $value === '') {
...
if (!function_exists('poado_claim_holder_sub')) {
    function poado_claim_holder_sub(string $a, string $b, int $scale = 9): string
    {
...
if (!function_exists('poado_claim_holder_add')) {
    function poado_claim_holder_add(string $a, string $b, int $scale = 9): string
    {
...
try {
    ...
    $csrf_ok = true;
    try {
        $r = csrf_check('rwa_claim_holder', $token);
     ...
     * Accepted inputs:
     *
     * claim_uids[]  OR JSON body: { "claim_uids": [...] }
     * claimed_tx_hash
     *
     * This endpoint marks claim rows as settled using a payout tx hash
     * produced by the treasury/admin payout process.
     */
    ...
    if (!$rows) {
        poado_claim_holder_exit([
            ...
            'message' => 'No matching holder claims found for current user.',
        ], 404);
    }
    ...
    $updateStmt = $pdo->prepare("
        UPDATE poado_rwa_holder_claims
        SET
            status = :status,
    ...
    foreach ($rows as $row) {
        $allocatedTon = poado_claim_holder_norm_decimal($row['allocated_ton'] ?? '0');
        $alreadyClaimedTon = poado_claim_holder_norm_decimal($row['claimed_ton'] ?? '0');
        $remainingTon = poado_claim_holder_sub($allocatedTon, $alreadyClaimedTon, 9);
        ...
        if ((float)$remainingTon <= 0 || in_array($status, ['claimed', 'void'], true)) {
            $skipped[] = [
                ...
                'message' => 'Claim row is not claimable.',
            ];
            continue;
        }

        $newClaimedTon = poado_claim_holder_add($alreadyClaimedTon, $remainingTon, 9);
        ...
        $totalClaimedNow = poado_claim_holder_add($totalClaimedNow, $remainingTon, 9);
    ...
    poado_claim_holder_exit([
        'ok' => true,
        'message' => 'Holder claim settlement processed.',
        'user' => [
            'id' => (int)$user['id'],
    ...
    poado_claim_holder_exit([
        'ok' => false,
        'error' => 'server_error',
        'message' => 'Failed to process holder claim.',
