...
if (function_exists('csrf_check')) {
    try {
        $r = csrf_check('swap_admin_reject', $csrf);
        $csrfOk = ($r !== false);
    } catch (Throwable $e) {
        $csrfOk = false;
    }
...
$stmt = $pdo->prepare("
    SELECT id, project_key, application_status
...
header('Location: /rwa/swap/admin/applications.php?status=rejected');
exit;